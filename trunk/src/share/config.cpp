...
int config_mode(int f, int w, int h)
{
	...
	SDL_DC_MapKey(0,SDL_DC_B,SDLK_F1);
	SDL_DC_MapKey(0,SDL_DC_X,SDLK_F2);
	SDL_DC_MapKey(0,SDL_DC_Y,SDLK_F3);
#endif
        return 1;
    }
...
int config_data_path(const char *path, const char *file)
{
    ...
    return 0;
#else
    strncpy(data_path, DATA_PREFIX, MAXSTR);
    return 1;
#endif
}
...
#else
    strncpy(user_path, USER_PREFIX, MAXSTR-1);
//    if (file)
  	  if (config_test(user_path, file))
    		return 1;
#endif
...
{
#ifdef DEBUG_AUDIO
    puts("audio: config_set_pause()");fflush(stdout);
#endif
    Mix_PauseMusic();
    paused = 1;
...
{
#ifdef DEBUG_AUDIO
    puts("audio: config_clr_pause()");fflush(stdout);
#endif
    Mix_ResumeMusic();
    paused = 0;
