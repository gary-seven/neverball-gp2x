...
#if defined(REDUCE_PHYSICS) && !defined(GAME_PUTT)
extern unsigned sol_test_inc;
extern unsigned sol_test_inc2;
//extern float sol_test_max;
#define MAX_TO_TEST 0.233333333f
//#define MAX_TO_TEST 0.222222222f
#else
#define MAX_TO_TEST 0.0f
#endif
...
static __inline__ void sol_body_v(float v[3],
...
static __inline__ void sol_body_p(float p[3],
                       const struct s_file *fp,
                       const struct s_body *bp)
{
    float v[3];
...
#else
    if (!(mp->fl & M_ADDITIVE))
        elementsBindTexture(GL_TEXTURE_2D, mp->o);
#endif

    if (mp->fl & M_ADDITIVE)
        elementsBlendFunc(GL_ONE, GL_ONE);
    else
                          ...
                          const struct s_bill *rp, float t)
{
    ...
    if (w > 0 && h > 0)
    {
        ...
        glPushMatrix();
        {
            ...
            {
                elementsTexCoord2f(0.0f, 1.0f); elementsVertex2f(-w / 2, y0);
                elementsTexCoord2f(1.0f, 1.0f); elementsVertex2f(+w / 2, y0);
                elementsTexCoord2f(1.0f, 0.0f); elementsVertex2f(+w / 2, y1);
                elementsTexCoord2f(0.0f, 0.0f); elementsVertex2f(-w / 2, y1);
            }
            elementsEnd();
        }
        glPopMatrix();
...
//static Uint32 poligonos=0;
//static Uint32 texturas=0;
                          ...
                          const struct s_geom *gp, short mi)
{
    ...
    {
//poligonos++;
        const GLfloat *ui = fp->tv[gp->ti].u;
        const GLfloat *uj = fp->tv[gp->tj].u;
        const GLfloat *uk = fp->tv[gp->tk].u;

#ifndef NO_LIGHT
        const GLfloat *ni = fp->sv[gp->si].n;
        const GLfloat *nj = fp->sv[gp->sj].n;
        const GLfloat *nk = fp->sv[gp->sk].n;
#endif

        const GLfloat *vi = fp->vv[gp->vi].p;
        const GLfloat *vj = fp->vv[gp->vj].p;
        const GLfloat *vk = fp->vv[gp->vk].p;

        elementsTexCoord2fv(ui);
#ifndef NO_LIGHT
        glNormal3fv(ni);
#endif
        elementsVertex3fv(vi);

        elementsTexCoord2fv(uj);
#ifndef NO_LIGHT
        glNormal3fv(nj);
#endif
        elementsVertex3fv(vj);

        elementsTexCoord2fv(uk);
#ifndef NO_LIGHT
        glNormal3fv(nk);
#endif
        elementsVertex3fv(vk);
    }
}
...
void sol_draw(const struct s_file *fp)
{
...
#endif
    {
	    ...
	    {
#ifdef NO_LIGHT
		const struct s_mtrl *mp = fp->mv + bi;
		glColor4f(0.5f+(mp->a[0]/2.0f),0.5f+(mp->d[0]/2.0f),0.5f+(mp->e[0]/2.0f),0.5f);
#endif
...
#ifdef NO_LIGHT
		const struct s_mtrl *mp = fp->mv + bi;
		glColor4f(0.5f+(mp->a[0]/2.0f),0.5f+(mp->d[0]/2.0f),0.5f+(mp->e[0]/2.0f),1.0f);
#endif
                          ...
                          const struct s_geom *gp, short mi)
{
    if (gp->mi == mi)
    {
        ...
        elementsTexCoord2f(vi[0], vi[2]);
        elementsVertex3fv(vi);

        elementsTexCoord2f(vj[0], vj[2]);
        elementsVertex3fv(vj);

        elementsTexCoord2f(vk[0], vk[2]);
        elementsVertex3fv(vk);
    }
}
...
static void sol_load_objects(struct s_file *fp, int s)
{
    ...
    for (i = 0; i < fp->bc; i++)
    {
        ...
        {
            fp->bv[i].ol = elementsGenLists(1);
            
            elementsNewList(fp->bv[i].ol, GL_COMPILE);
            {
        ...
        {
            fp->bv[i].tl = elementsGenLists(1);

            elementsNewList(fp->bv[i].tl, GL_COMPILE);
            {
        ...
        {
            fp->bv[i].rl = elementsGenLists(1);

            elementsNewList(fp->bv[i].rl, GL_COMPILE);
            {
        ...
        {
            fp->bv[i].sl = elementsGenLists(1);

            elementsNewList(fp->bv[i].sl, GL_COMPILE);
            {
...
{
    char new_image[MAXSTR];
    SDL_Surface *s;


    /* Check for a BMP, swapping channels if found. */
    strncpy(new_image, name, PATHMAX); strcat(new_image, ".bmp");
    if ((s = SDL_LoadBMP(config_data(new_image))))
    {
        image_swab(s);
        return s;
    }
    ...
    /* Check for a PNG. */
    strncpy(new_image, name, PATHMAX); strcat(new_image, ".png");
    if ((s = IMG_Load(config_data(new_image))))
        return s;

    /* Check for a TGA, swapping channels if found. */
    strncpy(new_image, name, PATHMAX); strcat(new_image, ".tga");
    if ((s = IMG_Load(config_data(new_image))))
    {
        image_swab(s);
        return s;
    ...
    /* Check for a JPG. */
    strncpy(new_image, name, PATHMAX); strcat(new_image, ".jpg");
    if ((s = IMG_Load(config_data(new_image))))
        return s;
...
int sol_load(struct s_file *fp, const char *filename, int k, int s)
{
    ...
    {
//poligonos=texturas=0;
//printf("sol_load '%s': ",filename);
        sol_load_file(fin, fp);
        sol_load_textures(fp, k);
        sol_load_objects (fp, s);

        fclose(fin);
//printf("%i texturas y %i poligonos\n",texturas,poligonos);
...
void sol_free(struct s_file *fp)
{
    ...
    for (i = 0; i < fp->bc; i++)
    {
        if (elementsIsList(fp->bv[i].ol))
            elementsDeleteLists(fp->bv[i].ol, 1);
        if (elementsIsList(fp->bv[i].tl))
            elementsDeleteLists(fp->bv[i].tl, 1);
        if (elementsIsList(fp->bv[i].rl))
            elementsDeleteLists(fp->bv[i].rl, 1);
    }
...
static __inline__ float sol_test_vert(float dt,
                           float T[3],
...
static __inline__ float sol_test_edge(float dt,
                           float T[3],
                           const struct s_ball *up,
                           const struct s_file *fp,
...
static __inline__ float sol_test_side(float dt,
                           float T[3],
                           const struct s_ball *up,
                           const struct s_file *fp,
...
static __inline__ float sol_test_fore(float dt,
                           const struct s_ball *up,
                           const struct s_side *sp,
                           const float o[3],
                           const float w[3])
...
static __inline__ float sol_test_back(float dt,
                           const struct s_ball *up,
                           const struct s_side *sp,
                           const float o[3],
                           const float w[3])
                           ...
                           const float w[3])
{
    ...
    if (up->r > MAX_TO_TEST) //sol_test_max)
    {
#if defined(REDUCE_PHYSICS) && !defined(GAME_PUTT)
        for (i = 0; i < lp->ec; i+=sol_test_inc)
#else
        for (i = 0; i < lp->ec; i++)
#endif
        {
...
static __inline__ float sol_test_body(float dt,
                           float T[3], float V[3],
                           const struct s_ball *up,
                           const struct s_file *fp,
                           const struct s_body *bp)
...
#if defined(REDUCE_PHYSICS) && !defined(GAME_PUTT)
    for (i = 0; i < fp->bc; i+=sol_test_inc2)
#else
    for (i = 0; i < fp->bc; i++)
#endif
    {
