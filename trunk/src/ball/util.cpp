...
static int util_coin_c[8]={ 0, 0, 0, 0, 0, 0, 0, 0 };
static int util_coin_n[8]={ 0, 0, 0, 0, 0, 0, 0, 0 };
static int util_coin_t[8]={ 0, 0, 0, 0, 0, 0, 0, 0 };
    ...
    if ((jd = gui_hstack(id)))
    {
        ...
        if ((kd = gui_vstack(jd)))
        {
            ...
            if ((ld = gui_hstack(kd)))
            {
                ...
                {
                    for (j = 0; j < n - 2; j++)
                        util_coin_c[j] = gui_count(md, 1000, GUI_SML, 0);

                    util_coin_c[j++] = gui_count(md, 1000, GUI_SML, GUI_SE);
                    gui_space(md);
                    util_coin_c[j++] = gui_count(md, 1000, GUI_SML, GUI_RGT);
                }

                if ((md = gui_vstack(ld)))
                {
#if 0
			util_coin_n[0] = gui_label(md, "Empty", GUI_SML, 0, gui_grn, gui_grn);
			util_coin_n[1] = gui_label(md, "Empty", GUI_SML, 0, gui_grn, gui_grn);
			util_coin_n[2] = gui_label(md, "Empty", GUI_SML, 0, gui_grn, gui_grn);
			gui_space(md);
			util_coin_n[3] = gui_label(md, "Empty", GUI_SML, 0, gui_red, gui_red);
#endif
			...
			const float *c1=gui_wht;
				...
				c0=c1=gui_red;
                        ...
                        util_coin_n[j] = gui_label(md, "Empty", GUI_SML, 0, c0, c1);
		    }
                }
                ...
                {
                    for (j = 0; j < n - 2; j++)
                        util_coin_t[j] = gui_clock(md, 359999, GUI_SML, 0);

                    util_coin_t[j++] = gui_clock(md, 359999,  GUI_SML, GUI_SW);
                    gui_space(md);
                    util_coin_t[j++] = gui_clock(md, 359999,  GUI_SML, GUI_LFT);
                }
            }
        }
        gui_filler(jd);
...
void set_most_coins(int level, int n)
{
    ...
    for (j = 0; j < n; j++)
    {
        gui_set_count(util_coin_c[j], level_coin_c(level, j));
        gui_set_label(util_coin_n[j], level_coin_n(level, j));
        gui_set_clock(util_coin_t[j], level_coin_t(level, j));
    }
}

/*---------------------------------------------------------------------------*/

static int util_time_c[8]={ 0, 0, 0, 0, 0, 0, 0, 0 };
static int util_time_n[8]={ 0, 0, 0, 0, 0, 0, 0, 0 };
static int util_time_p[8]={ 0, 0, 0, 0, 0, 0, 0, 0 };
    ...
    if ((jd = gui_hstack(id)))
    {
        ...
        if ((kd = gui_vstack(jd)))
        {
            ...
            if ((ld = gui_hstack(kd)))
            {
                ...
                {
                    for (j = 0; j < n - 2; j++)
                        util_time_p[j] = gui_clock(md, 359999, GUI_SML, 0);

                    util_time_p[j++] = gui_clock(md, 359999, GUI_SML, GUI_SE);
                    gui_space(md);
                    util_time_p[j++] = gui_clock(md, 359999, GUI_SML, GUI_RGT);
                }
			...
			const float *c1=gui_wht;
				...
				c0=c1=gui_red;
                        if (j == n - 1) gui_space(md);
                        util_time_n[j] = gui_label(md, "Empty", GUI_SML, 0, c0, c1);
		   } 
                }
                ...
                {
                    for (j = 0; j < n - 2; j++)
                        util_time_c[j] = gui_count(md, 1000, GUI_SML, 0);

                    util_time_c[j++] = gui_count(md, 1000, GUI_SML, GUI_SW);
                    gui_space(md);
                    util_time_c[j++] = gui_count(md, 1000, GUI_SML, GUI_LFT);
                }
            }
        }
        gui_filler(jd);
...
void set_best_times(int level, int n)
{
    ...
    for (j = 0; j < n; j++)
    {
        gui_set_clock(util_time_p[j], level_time_t(level, j));
        gui_set_label(util_time_n[j], level_time_n(level, j));
        gui_set_count(util_time_c[j], level_time_c(level, j));
    }
}
