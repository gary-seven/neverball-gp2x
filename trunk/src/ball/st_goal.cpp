...
static char goal_player[MAXNAM];
...
static int goal_action(int i)
{
    ...
    config_get_s(CONFIG_PLAYER, goal_player, MAXNAM);
    l = strlen(goal_player);
    ...
    case GUI_BS:
        if (l > 0)
        {
            ...
            config_set_s(CONFIG_PLAYER, goal_player);
            level_name(curr_level(), goal_player, time_i, coin_i);
    ...
    default:
        if (l < MAXNAM - 1)
        {
            goal_player[l + 0] = gui_keyboard_char((char) i);
            goal_player[l + 1] = 0;
...
static int goal_enter(void)
{
    ...
    if ((id = gui_vstack(0)))
    {
        ...
        if (high)
            gid = gui_label(id, "New Record", GUI_MED, GUI_ALL, gui_grn, gui_grn);
        else
            gid = gui_label(id, "GOAL", GUI_LRG, GUI_ALL, gui_blu, gui_grn);
