...
static int course_enter(void)
{
    ...
    audio_music_fade_to(0.5f, "bgm/inter.mod");
...
static void next_stick(int id, int a, int v)
{
    if (config_tst_d(CONFIG_JOYSTICK_AXIS_X, a))
	    gui_pulse(gui_stick(id, v, 0), 1.2f);
    if (config_tst_d(CONFIG_JOYSTICK_AXIS_Y, a))
...
static int dx_to_set_rot=0;;
static int dy_to_set_mag=0;;

static void stroke_paint(int id, float st)
{
	game_set_rot(dx_to_set_rot);
	game_set_mag(dy_to_set_mag);
    game_draw(0);
    hud_paint();
}
